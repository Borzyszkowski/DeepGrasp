...
from core.mesh_visualization.visual_utils import euler
...
class MeshViewer(object):
    ...
    def save_snapshot(self, save_path):
        if not self.offscreen:
            print('We do not support snapshot rendering in Interactive mode!')
            return
    ...
    def save_recording(self, save_path, recording_name="/recording.avi", fps=40):
        ...
        if not self.offscreen:
            print('We do not support video rendering in Interactive mode!')
            return
        ...
        print(f"Video saved in: {save_path + output_recording}")

    def start_gif_recording(self):
        """ Starts recording gif in pyrender. """
        if self.offscreen:
            print('We do not support gif rendering in Offscreen mode!')
            return
    ...
    def end_gif_recording(self, save_path, recording_name="/recording.gif"):
        ...
        print(f"Gif saved in: {save_path + recording_name}")
